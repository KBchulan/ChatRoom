...
- 登录时先判断用户是否存在，若不存在则直接返回错误

### [2025-12-17] 安全增强

- 引入 jwt-cpp 库，新增 JWT 组件用于生成和验证 token
- 新增基于 Redis 滑动窗口的限流组件，默认每分钟 100 次请求
- 重构 connection.cc：
  - 从 lambda 改为成员函数，代码更清晰
  - 支持 HTTP keep-alive，避免频繁创建连接
  - 增加 JWT 校验中间件，非白名单路由需携带 Authorization 头
  - 增加限流检查，超限返回 429 状态码
