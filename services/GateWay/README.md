...
```bash
Usage: GateWay [-h] [-p <port>]
Options:
  -h, --help         显示帮助信息
  -p, --port <port>  服务器端口 (默认: 10001)
```
...
```
Request → [服务可用检查] → [限流检查] → [URL解析] → [请求ID生成] → [JWT校验] → [业务逻辑] → Response
              ↓                ↓              ↓              ↓              ↓
           405 降级        429 拒绝      500 错误      500 错误      401 未授权
```

服务降级通过 `AVAILABLE_ROUTES` 白名单实现，不在白名单中的路由返回 405。
